...
console.log('Target user: bdhlami883@gmail.com');
...
async function deleteUser() {
  ...
  try {
    ...
    const checkResult = await client.query(
      ...
      ['bdhlami883@gmail.com']
    );
    
    if (checkResult.rows.length === 0) {
      console.log('❌ User bdhlami883@gmail.com not found in database');
      return;
    }
    ...
    for (const table of tables) {
      try {
        ...
        console.log(`   ✅ Deleted from ${table.name}: ${result.rowCount || 0} rows`);
      } catch (error) {
    ...
    console.log('\n✅ User bdhlami883@gmail.com deleted successfully!');
