...
async function fixMissingTablesAndColumns() {
  ...
  try {
    ...
    // Ensure test_pass_rate is INTEGER if it exists as DECIMAL
    try {
      await client.query(`
        ALTER TABLE sprints 
        ALTER COLUMN test_pass_rate TYPE INTEGER USING COALESCE(test_pass_rate::INTEGER, 0)
      `);
      console.log(`  ✅ Updated test_pass_rate to INTEGER`);
    } catch (err) {
      // Column might not exist or already be INTEGER
      console.log(`  ℹ️  test_pass_rate: ${err.code === '42703' ? 'Column does not exist, will be created' : err.message}`);
    }
